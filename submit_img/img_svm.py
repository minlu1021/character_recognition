...
class IMG_SVM:
	...
	def split_data(self):
		...
		self.train_X, self.test_X, self.train_Y, self.test_Y = train_test_split(self.all_X, self.all_Y, test_size = 0.15)
...
#		clf = SVC(kernel = 'linear', decision_function_shape = 'ovr') # 80%
	...
	def predict_test(self):
		...
		ax=sns.heatmap(inp,annot=False,edgecolors="black",mask=inp==1,vmin=0,vmax=1)
		matplotlib.pyplot.show(ax)
		f = open("confusion_mat_img.txt", 'w')
		l = con_matrix.tolist()
